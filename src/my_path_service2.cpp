...
bool callback(example_ros_service::PathSrvRequest& request, example_ros_service::PathSrvResponse& response)
{
    ...
    double yaw_desired, yaw_current, x_desired,y_desired,x_current, y_current, dx, dy, travel_distance, spin_angle;
    ...
    for (int i=0;i<npts;i++) { //visit each subgoal
 	...
 	//get_yaw_and_dist(g_current_pose, pose_desired,travel_distance, yaw_desired);
        x_desired = pose_desired.position.x;    
        y_desired = pose_desired.position.y;
	x_current = g_current_pose.position.x;
	y_current = g_current_pose.position.y;
	dx = x_desired - x_current;
	dy = y_desired - y_current;
	travel_distance = sqrt(dx*dx + dy*dy);
	//yaw_desired = atan2(dy, dx);
        ...
        g_current_pose.position.x = pose_desired.position.x;
	 g_current_pose.position.y = pose_desired.position.y;
